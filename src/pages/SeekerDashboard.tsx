      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-1 p-1 bg-white rounded-xl border border-gray-200 mb-6 overflow-x-auto">
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}>
              {tab.icon}
              {tab.label}
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
                ...
                <div className="space-y-3">
                  {myApps.slice(0, 4).map(app => (
                    <div key={app.id} className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl">
                      <div className="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center shrink-0 text-sm font-bold text-gray-400">
                        {app.companyName[0]}
                      </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-gray-900">وظائف مقترحة</h2>
        ...
        {activeTab === 'applications' && (
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-5">طلبات التقديم ({myApps.length})</h2>
            {myApps.length === 0 ? (
              ...
              <div className="space-y-4">
                {myApps.map(app => (
                  <div key={app.id} className={`flex flex-col sm:flex-row sm:items-center gap-4 p-5 border rounded-2xl ${app.status === 'pending_approval' ? 'border-amber-200 bg-amber-50/30' : 'border-gray-200'}`}>
                    <div className="w-12 h-12 rounded-xl bg-gray-100 border border-gray-200 flex items-center justify-center shrink-0 text-lg font-bold text-gray-400">
                      {app.companyName[0]}
                    </div>
                    <div className="flex-1">
        ...
        {activeTab === 'saved' && (
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-5">الوظائف المحفوظة ({savedJobs.length})</h2>
            {savedJobs.length === 0 ? (
              <div className="text-center py-16">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {savedJobs.map(job => (
                  <Link key={job.id} to={`/jobs/${job.id}`} className="flex items-start gap-3 p-4 border border-gray-200 rounded-xl hover:border-blue-300 hover:bg-blue-50 transition-all">
