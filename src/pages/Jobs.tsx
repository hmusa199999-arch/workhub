...
import { Search, MapPin, SlidersHorizontal, X, ChevronDown, Plus, Phone, Briefcase, User } from 'lucide-react';
import JobCard from '../components/JobCard';
import PostSeekerAdModal from '../components/PostSeekerAdModal';
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        {/* ── Job Seekers Section ── */}
        <div className="mt-14 border-t border-red-900/20 pt-10">
          <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => user ? setShowSeekerModal(true) : navigate('/login')}
          ...
          {seekerAds.length > 0 && (
            <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 ${!showSeekers ? 'max-h-[420px] overflow-hidden relative' : ''}`}>
              {!showSeekers && seekerAds.length > 3 && (
              ...
              {(showSeekers ? seekerAds : seekerAds.slice(0, 6)).map(ad => (
                <div key={ad.id}
                  className="bg-gray-900 border border-gray-800 hover:border-red-900/50 rounded-2xl p-5 transition-all hover:shadow-lg hover:shadow-red-900/10">
                  <div className="flex items-start gap-3 mb-3">
                    <div className="w-11 h-11 rounded-full bg-gradient-to-br from-red-700 to-red-900 flex items-center justify-center font-black text-white text-lg shrink-0">
                      {ad.name[0]}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-white text-sm truncate">{ad.name}</h3>
                      <p className="text-red-400 text-xs font-semibold mt-0.5 truncate">{ad.jsTitle}</p>
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {ad.jsSector && (
                      <span className="flex items-center gap-1 px-2 py-0.5 bg-red-900/30 border border-red-800/40 text-red-300 text-xs rounded-full">
                        <Briefcase className="w-3 h-3" /> {ad.jsSector}
                      </span>
                    )}
                    {ad.jsExp && (
                      <span className="px-2 py-0.5 bg-gray-800 text-gray-400 text-xs rounded-full">{ad.jsExp}</span>
                    )}
                    {ad.jsType && (
                      <span className="px-2 py-0.5 bg-gray-800 text-gray-400 text-xs rounded-full">{ad.jsType}</span>
                    )}
                    {ad.location && (
                      <span className="flex items-center gap-1 px-2 py-0.5 bg-gray-800 text-gray-400 text-xs rounded-full">
                  ...
                  {ad.desc && (
                    <p className="text-gray-500 text-xs leading-relaxed mb-3 line-clamp-2">{ad.desc}</p>
                  )}
